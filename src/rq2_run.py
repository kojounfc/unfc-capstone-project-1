...
def build_customer_erosion(item_df: pd.DataFrame) -> pd.DataFrame:
    ...
    _require_columns(
        ...
        context="build_customer_erosion",
    )
...
def run_rq2(
    ...
    # Diagnostics
    k_list_elbow = list(range(1, max(2, k_max) + 1))
    elbow_df = elbow_inertia_over_k(X_scaled, k_list=k_list_elbow, random_state=42)

    k_list_sil = list(range(max(2, k_min), max(3, k_max) + 1))
    silhouette_df = silhouette_over_k(X_scaled, k_list=k_list_sil, random_state=42)
...
def _parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Run RQ2 concentration + segmentation pipeline."
    )
    p.add_argument(
        "--input-parquet",
        type=str,
    ...
    p.add_argument(
        "--k-max", type=int, default=10, help="Max k for diagnostics (default 10)."
    )
    p.add_argument(
        "--top-x",
        type=float,
